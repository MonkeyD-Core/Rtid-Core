This directory contains the RtidcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Rtidcoin UI.
...
ClientModel is used by the main application `rtidcoingui` and several models like `peertablemodel`.
...
Handles URI based application switching (e.g. when following a rtidcoin:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: RTID / mRTID / etc handling
* `callback.h`
...
10. Start debugging with Qt Creator (you might need to the executable to "itcoin-qt" under "Run", which is where you can also add command line arguments)
