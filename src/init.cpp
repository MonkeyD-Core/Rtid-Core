...
public:
    ...
    void RegisterRPC(RTIDCTable &) const override {}
 ...
 * between rtidcoind, and rtidcoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/rtidcoin/rtidcoin>";
    const std::string URL_WEBSITE = "<https://rtidcoin.org>";
...
/** Sanity checks
 *  Ensure that Rtidcoin is running in a usable environment with all
...
{
    // Make sure only a single Rtidcoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if rtidcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if rtidcoin is started while in a temporary directory.\n",
