...
The headless daemon `rtidcoind` has the JSON-RPC API enabled by default, the GUI
`rtidcoin-qt` has it disabled by default. This can be changed with the `-server`
