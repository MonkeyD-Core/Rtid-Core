# PSBT Howto for Rtidcoin Core

Since Rtidcoin Core 2.0.0, an RPC interface exists for Partially Signed Rtidcoin
...
PSBT is an interchange format for Rtidcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Rtidcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Rtidcoin transaction (in network format)
...
#### Multisig with multiple Rtidcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Rtidcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `cranepartidcoinis used.
...
Later, when *V* RTID has been received on *Amulti*, and Bob and Carol want to
